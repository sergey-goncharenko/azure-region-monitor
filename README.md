...
Set `AZURE_VM_SKUS=all` to track every SKU returned by `az vm list-skus --resource-type virtualMachines --all`, filtered into regions by each SKU's locations metadata:
