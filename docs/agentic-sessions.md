...
- The CLI runs in offline mode with built-in MCP servers and remote control disabled. Issue coding uses local shell-assisted autopilot for repository search, diffs, and tests, while explicit tool denials block PowerShell, `git push`, GitHub/Azure CLI, downloads, and package installation. The Azure deployment's 32K prompt window cannot retain repeated multi-hundred-line file dumps: a live canary compacted 31 times and restarted the same inspection loop. Coding sessions therefore hide Copilot's high-volume `view`/`read` tools and direct the agent to use `rg` plus focused local shell extracts of at most 120 lines. The full checkout remains readable; only bulk tool output is constrained. Provider/GitHub secrets are filtered from tool subprocesses. Private report sessions still exclude shell entirely. Deterministic scope and validation gates remain outside the model.
