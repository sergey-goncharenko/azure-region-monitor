...
- Candidate changes are buffered as artifacts. Before a PR can be written, threat detection checks the patch and a deterministic post-step applies it to a clean checkout and runs `python scripts/check.py`. That script is the single validation entrypoint shared with [.github/workflows/pr-validation.yml](../.github/workflows/pr-validation.yml) and with the agent's own pre-commit `python scripts/check.py --fix`, so the three cannot drift apart. A candidate patch that edits `scripts/check.py` is rejected, because that file is the gate. Generated `data/**` and `public/api/**` files are excluded; protected manifests, instructions, and `.github/**` changes create a blocking review requirement.
