...
- AKS extension catalog and Azure AI model catalog probes discover their feature items from regional Azure catalogs/lists. VM SKU probes discover sizes from regional `az vm list-skus --location <region> --resource-type virtualMachines --all` calls, supplemented by legacy `az vm list-sizes --location <region>` when that fallback adds read-only listing evidence. Full and VM-focused workflows pass `AZURE_VM_SKUS=all`, so VM SKU rows cover every listed size from those regional catalogs. The full run unions discovered items and fills absent items as `unavailable` where the catalog probe succeeded.
