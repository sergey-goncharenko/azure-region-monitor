...
- AKS extension catalog and Azure AI model catalog probes discover their feature items from regional Azure catalogs/lists. VM SKU probes discover sizes from `az vm list-skus --resource-type virtualMachines --all` and filter them into regions by each SKU's locations metadata. Full and VM-focused workflows pass `AZURE_VM_SKUS=all`, so VM SKU rows cover every listed size from that catalog. The full run unions discovered items and fills absent items as `unavailable` where the catalog probe succeeded.
