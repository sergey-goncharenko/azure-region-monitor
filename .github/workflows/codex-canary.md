...
If you implemented a coherent correction, call `create_pull_request` exactly once. Publish even when validation or threat detection reports findings: the draft PR, warning label, and generated `REQUEST_CHANGES` review keep the evidence visible for a human decision. Never weaken a check or hide a finding to make publication look clean.
