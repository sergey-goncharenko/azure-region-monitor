...
- Scheduled Azure-funded issue work is configured by `.github/workflows/scheduled-azure-backlog.yml` and documented in `docs/agentic-sessions.md`. It uses pinned OpenCode with Azure OpenAI `o4-mini` for one eligible GitHub issue by default (manual runs may choose one to three) and does not run documentation alignment. Recurring unknown work is enriched from the current live snapshot and remains open after generated PRs merge. Keep all tasks bounded: no Azure create/delete probes and no manual edits to generated live snapshot data. The Copilot cloud-agent workflow remains manual-only.
- Each issue-agent may implement one atomic, independently reviewable slice, with 24 OpenCode steps and a 30-minute outer budget. It can read/search the full repository, but OpenCode edit permissions and deterministic outer code both restrict writes to issue-derived source/test paths. Shell, network, external-directory, subagent, and question tools are denied. The agent must begin a justified edit by step 12 rather than wait for pre-existing failing tests. At timeout, an in-scope partial diff is retained only after focused tests, Ruff, and whitespace validation pass. A no-PR result is acceptable and replaces one stable issue note.
