...
- Scheduled Azure-funded issue work is configured by `.github/workflows/scheduled-azure-backlog.yml` and documented in `docs/agentic-sessions.md`. It uses GitHub Copilot CLI with Azure OpenAI BYOK for one eligible GitHub issue by default (manual runs may choose one to three) and does not run documentation alignment. Recurring unknown work is enriched from the current live snapshot and remains open after generated PRs merge. Keep all tasks bounded: no Azure create/delete probes and no manual edits to generated live snapshot data. The Copilot cloud-agent workflow remains manual-only.
- Scheduled issue work defaults to one issue per day. Each issue-agent runs as a standard repository-instruction-aware Copilot prompt session and may implement only one atomic, independently reviewable slice, with a 40-minute model budget and three autopilot continuations. The model may inspect the full repository through search and the enforced four-call bounded reader, but high-volume file-dump tools are hidden to avoid repeated context compaction; deterministic outer code allows writes only inside the issue-derived source/test scope. At timeout, an in-scope partial diff is retained only after focused tests, Ruff, and whitespace validation pass. A no-PR result is acceptable: outer code must replace a stable issue note with the outcome/decomposition instead of continuing exhaustive work or adding repeated comments.
