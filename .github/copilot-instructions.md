...
- Scheduled Azure-funded issue work is configured by `.github/workflows/scheduled-azure-backlog.yml` and documented in `docs/agentic-sessions.md`. It uses GitHub Copilot CLI with Azure OpenAI BYOK for two eligible GitHub issues by default (manual runs may choose one to three) and does not run documentation alignment. Recurring unknown work is enriched from the current live snapshot and remains open after generated PRs merge. Keep all tasks bounded: no Azure create/delete probes and no manual edits to generated live snapshot data. The Copilot cloud-agent workflow remains manual-only.
- Each issue-agent session may implement only one atomic, independently reviewable slice, with a 20-minute model budget and three autopilot continuations. The model may inspect the full repository, but deterministic outer code allows writes only inside the issue-derived source/test scope. A no-PR result is acceptable: outer code must replace a stable issue note with the outcome/decomposition instead of continuing exhaustive work or adding repeated comments.
