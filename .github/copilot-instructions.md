...
- Scheduled Azure-funded issue work is configured by `.github/workflows/scheduled-azure-backlog.yml` and documented in `docs/agentic-sessions.md`. It uses pinned Aider with full Azure OpenAI `gpt-4o` for one eligible GitHub issue by default (manual runs may choose one to three) and does not run documentation alignment. Recurring unknown work is enriched from the current live snapshot and remains open after generated PRs merge. Keep all tasks bounded: no Azure create/delete probes and no manual edits to generated live snapshot data. The Copilot cloud-agent workflow remains manual-only.
- Each issue-agent may implement one atomic, independently reviewable slice through Aider's architect/editor flow and a 15-minute outer budget. Full GPT-4o plans, then full GPT-4o applies SEARCH/REPLACE edits. Aider receives a full repository map but only issue-derived source/test paths as editable files. Shell suggestions, auto-commits, URL tools, and remote analytics are disabled. Deterministic outer code rejects out-of-scope changes and runs focused tests, Ruff, and whitespace validation. A no-PR result is acceptable and replaces one stable issue note.
