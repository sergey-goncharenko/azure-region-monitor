...
- `.github/workflows/scheduled-azure-maintenance.yml` runs three separate BYOK sessions after the backlog window: documentation alignment, read-only security analysis, and read-only repository-hygiene recommendations. Documentation may edit only `README.md`, `.github/copilot-instructions.md`, and `docs/agentic-sessions.md`. Analysis sessions must publish only stable `azure-agent-report` issues through deterministic outer code; they must never edit repository files or delete branches/worktrees.
