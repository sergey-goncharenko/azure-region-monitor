...
- Each issue-agent may implement one atomic, independently reviewable slice through one reasoning `o4-mini` Aider SEARCH/REPLACE pass and a 15-minute outer budget. If focused or full tests fail, outer code may provide sanitized failure output for exactly one bounded repair pass. Aider receives a full repository map but only issue-derived source/test paths as editable files. Shell suggestions, auto-commits, URL tools, and remote analytics are disabled. Deterministic outer code rejects out-of-scope changes and runs focused tests, the full suite, Ruff, and whitespace validation. An initial no-PR result is acceptable and replaces one stable issue note.
...
- Bot PR rework is dispatched by `.github/workflows/azure-pr-rework.yml` when a write-level collaborator submits **Request changes** or starts a PR conversation comment with `/agent-rework`. The triggering review body or command suffix becomes bounded trusted acceptance criteria but cannot expand the source issue's derived scope or override tool/security controls. Rework must update the same `azure-issues/issue-<number>` branch, deduplicate active requests, and keep Azure credentials exclusively in the scheduled BYOK workflow. A no-change or empty cumulative rework diff is a failed rework, never a successful status.
